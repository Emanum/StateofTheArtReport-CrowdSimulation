...
As we already outlined in Section \ref{chap:key_areas}, the behavioral model is one of the core component of an believable crowd simulation. In the following Section we want to compare two groups of approaches rule- and force-based models and automated models, using some papers as examples.
...
According to the authors from the HiDAC System, Helbing's social force based system (see Section \ref{chap:helbing}) has one problem. Agents appear to vibrate in high density areas or when an agent is in a hurry. In addition the authors also wanted to achieve other believable crowd behavior which include: 
\begin{itemize}
